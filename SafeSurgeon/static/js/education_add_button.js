...
document.addEventListener('DOMContentLoaded', function(){
    ...
    const managementForm = document.querySelector('#education-management-form>input[name$=TOTAL_FORMS]');

    if  (addEducationButton && educationFormsContainer && managementForm) {
        addEducationButton.addEventListener('click', function() {
            let formCount = parseInt(managmnetForm.value);
           
            if (educationFormsContainer.children.length>1){
