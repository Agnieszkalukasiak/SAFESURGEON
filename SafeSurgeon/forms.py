...
class ClinicForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # form to be valid even if empty
        if clinic or new_clinic_name or self.cleaned_data.get('DELETE'):
            return cleaned_data
        else:
            raise forms.ValidationError(
                "Please either select an existing "
                "clinic or enter a new clinic name."
            )

        # If a new clinic name is provided, clean it up for saving
        if new_clinic_name:
            cleaned_data['new_clinic_list'] = [new_clinic_name.strip()]

        return cleaned_data

    def save(self, surgeon, city, commit=True):
        # Empty list of clinics to associate with the surgeon
        clinics = []

        # Handles existing clinic
    ...
    # Handles new clinic
        ...
        if commit and clinics:
            # Clearing existing relationships to avoid duplicates
            Surgeon.clinic.through.objects.filter(surgeon=surgeon).delete()

        # Create new relationships
            for clinic in clinics:
                Surgeon.clinic.through.objects.create(
                    surgeon=surgeon,
                    clinic=clinic
                )
...
class EducationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'institution_country': forms.Select(
